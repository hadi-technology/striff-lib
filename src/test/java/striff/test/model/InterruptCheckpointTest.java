...
/**
 * Cooperative cancellation for the model-merge and relation-extraction phases.
 *
 * <p>On a very large model both {@link CodeDiff}'s merge and {@link ExtractedRelationships} used to
 * walk tens of thousands of components with no interrupt check, so a caller's
 * {@code Thread.interrupt()} was ignored and the work ran to completion regardless. These tests pin
 * the two halves of the fix: an interrupted run aborts promptly with a {@link CancellationException},
 * and an ordinary (never-interrupted) run produces exactly the relations it did before -- the
 * checkpoints are inert unless the thread is interrupted.
 */
