...
public class CodeDiff implements Serializable {
     ...
     * caller enforcing an analysis deadline (striff-api #289) otherwise sits past its interrupt --
     * the pod then only recovers via a heavy JVM self-exit. This mirrors the cooperative-cancellation
     * checkpoint that {@code OOPSourceCodeModel.merge} performs in clarpse: checking every few hundred
     ...
     *
     * <p>Cooperative cancellation: an analysis deadline interrupts this thread, and the model merge is
     * one of the CPU wedges that would otherwise ignore the request (striff-api #289). Re-asserts the
     * interrupt flag before throwing so callers can still observe it, then throws an unchecked
