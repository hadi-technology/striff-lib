...
Striff-lib provides structural diff diagrams for code reviews. The core entry point is `StriffOperation`, which supports two construction modes:

1. **Full Pipeline** - Parse source files, compute diffs, and render diagrams
2. **Render-Only** - Re-use an already-computed `CodeDiff` for multiple renders
...
3. **Full parsing** - For large codebases, parsing dominates the execution time.
...
* Caller must explicitly manage intermediate state (CodeDiff, compileFailures)
